...
- **Source-derived Flappy Bird** - The standalone Flappy Bird app adapts VadimBoev/FlappyBird `v1.9.2` into a silent native DC32 port with the original visible PNG sprites, full-width landscape playfield, source-matched pipe/bird physics, result panel, medals, and `/SAVE/flappy.sav` best-score persistence. Use A, Up, or Start to flap/start/restart, and Center to exit.
...
| `src/apps/flappy/` | VadimBoev/FlappyBird-derived silent port with generated visible sprites, full-width landscape playfield, source-matched bird/pipe behavior, result panel, medals, and `/SAVE/flappy.sav` best-score persistence. |
