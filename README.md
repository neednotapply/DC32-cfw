...
- **Tool shell and recovery** - Boots to `DC32-cfw`, shows a Main Menu, and uses boot guard state to recover to the menu after a reset or hard fault inside a tool. The header shows a filtered battery percentage with low-battery and USB-power coloring. Crash recovery can show the failed mode plus fault registers.
- **Main Menu** - Provides File Browser, Universal IR, USB, Media, Games, Settings, and Power Off entries. USB groups USB Storage, USB Keyboard, BadUSB, Autoclicker, and USB Gamepad. Media groups Music, Image Viewer, Starfield, Spiro, and Cube. Games groups Emulators and Ports; Ports groups Pong, Tetris, Arkanoid, Flappy Bird, T-Rex Runner, DOOM shareware, Chip's Challenge, Scorched Earth, Pipe Dream, Sokoban, and Jazz Jackrabbit shareware. While an emulator or port is running, FN exposes live Audio, Screen, and LED settings or returns to the main menu.
- **Pong Arcade** - The standalone Pong app uses one shared fixed-point paddle/ball engine with Classic, Doubles, Quadrapong, Super Pong, Rebound, Breakout, Warlords-lite, and Power Pong rule modules. Its silent menu includes Classic/Teams/Rainbow color settings; shared mute and volume are available from FN > Audio. Use the D-pad to move, A to select or restart, B to return to the mode menu, and FN for the shared Ports menu.
- **NullpoMino-derived Tetris** - The standalone Tetris app provides 150-line Marathon, 40-Line Race, and two-minute Ultra with Standard, Standard Fast-B, and Nintendo-R rules. It includes hold/next/ghost behavior where supported, SRS kicks, spins, B2B, combos, persistent per-rule records, and buzzer SFX governed by the shared Audio setting. During play use A/B to rotate, Select to hold, Up to hard-drop, Start to pause, and FN for the shared Ports menu.
- **Source-derived Flappy Bird** - The standalone Flappy Bird app adapts VadimBoev/FlappyBird `v1.9.2` into a silent native DC32 port with the original visible PNG sprites, full-width landscape playfield, source-matched pipe/bird physics, result panel, medals, and `/SAVE/PORTS/flappy.sav` best-score persistence. Use A, Up, or Start to flap/start/restart, and FN for the shared Ports menu.
...
- **Categorized app menus** - Launches resident tools and standalone `/APPS/*.DC32` apps from USB, Media, and Games categories. Standalone SD apps include Pong, Tetris, Arkanoid, Flappy Bird, T-Rex Runner, DOOM shareware, Chip's Challenge, Scorched Earth, Pipe Dream, Sokoban, Jazz Jackrabbit shareware, Starfield, Spiro, and Cube.
...
- **Music player** - Plays `.rtttl`, RTTTL `.txt`, and PCM `.wav` files from `/MUSIC` or from the browser, with folder navigation, progress display, play/pause, previous/next, per-track loop, and access to the shared volume.
...
- **Settings** - Persists Emulator, Audio, LED, Screen, USB, and Music-loop settings in QSPI flash. Audio provides a universal mute and 0-15 master volume for music, emulators, ports, and system sounds. Emulator settings include per-console upscaling and display speed (`50%`, `100%`, `150%`, `200%`), plus Game Boy palette selection. Screen settings include rotation and brightness. LED settings include pattern, color, custom RGB, speed, and brightness.
...
| `/SAVE` | Parent folder for console-specific save folders: `/SAVE/AB`, `/SAVE/GB`, `/SAVE/GBC`, and `/SAVE/NES`, plus `/SAVE/PORTS` for native port saves. Emulator saves use the original ROM filename stem with `.sav`; cleaned/detected-title and 8.3 fallback files from earlier builds are still read. |
| `/APPS` | SD-loaded app binaries built by this repo, including emulators, tools, `doom.DC32` plus `doom1.whx`, `chips.DC32` plus `chips-tworld.pak`, `scorch.DC32` plus `scorch-xscorch.pak`, `pipe.DC32` plus `pipe-pipedreamer.pak`, `sokoban.DC32` plus `sokoban-xsokoban.pak`, and standalone apps such as Pong, Tetris, Arkanoid, Flappy Bird, T-Rex Runner, Starfield, Spiro, and Cube. |
...
| `src/apps/port/` | Shared support for faithful source-derived ports: scratch heap, RGB332/paletted renderer bridge, FAT pack reads, save helpers, and live FN-menu handling. |
| `src/apps/tetris/` | NullpoMino-derived fixed-step Tetris engine with three modes, three rule presets, persistent rankings/settings, badge-native rendering, and buzzer SFX. |
| `src/apps/arkanoid/` | wkeeling/arkanoid-derived five-round port with a centered portrait arena, split side HUD, original scaled sprites, enemies, power-ups, and `/SAVE/PORTS/arkanoid.sav` high-score persistence. |
| `src/apps/flappy/` | VadimBoev/FlappyBird-derived silent port with generated visible sprites, full-width landscape playfield, source-matched bird/pipe behavior, result panel, medals, and `/SAVE/PORTS/flappy.sav` best-score persistence. |
| `src/apps/trex/` | Wayou/Chromium T-Rex Runner port with generated original sprite assets, source-matched physics and obstacle timing, night mode, and `/SAVE/PORTS/trex.sav` high-score persistence. |
...
| `tools/build_flappy_assets.py` | Fetches and verifies the pinned VadimBoev/FlappyBird source archive, then generates pre-scaled RGB332 sprites linked into `flappy.DC32`. |
...
- OpenJazz uses badge-native controls: A selects in menus, B goes back, Start toggles quick pause, Select cycles weapons, and FN opens the shared Ports menu. JJ1 save slots preserve the selected level and difficulty; loading a save restarts that level rather than restoring an exact mid-level position.
- Tetris, Flappy Bird, T-Rex Runner, Chip's Challenge, Scorched Earth, Pipe Dream, and Sokoban now ship as source-derived/data-compatible ports. Tetris adapts NullpoMino commit `4de098dd0b48d991247313d8dba30b9721e6f9d9` and persists records/settings in `/SAVE/PORTS/tetris.sav`; Flappy Bird adapts VadimBoev/FlappyBird `v1.9.2` and persists its best score in `/SAVE/PORTS/flappy.sav`; T-Rex uses the pinned Wayou/Chromium LDPI sprite and persists its high score in `/SAVE/PORTS/trex.sav`; `CHIPS.DAT` remains user-provided and is not redistributed.
...
- Tetris rules and mode reference: NullpoMino's BSD-licensed action-puzzle engine at pinned commit `4de098dd0b48d991247313d8dba30b9721e6f9d9`.
- Flappy Bird port reference: VadimBoev/FlappyBird `v1.9.2` at pinned commit `3b3060cdf6b38b819e5a649bc92d11776decd0b4`; upstream notes that Flappy Bird rights and resources belong to DotGEARS.
