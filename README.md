...
| `src/Makefile` | Build rules for generating the badge firmware binary, UF2 image, and direct-programming helpers. |
...
The GitHub Actions workflow packages a `firmware` artifact after it builds a commit. GitHub releases also receive the end-user update file:

- `DC32-cfw.uf2` - recommended RP2350 USB bootloader image. Copy this only to the mounted UF2 bootloader drive.

The build still creates `DC32-cfw.bin` internally as the raw firmware image for UF2 generation and direct programmers, but release packages publish the UF2 users should copy to a badge. Use one flashing path per update:
