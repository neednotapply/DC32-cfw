...
- **Pong Arcade** - The standalone Pong app uses one shared fixed-point paddle/ball engine with Classic, Doubles, Quadrapong, Super Pong, Rebound, Breakout, Warlords-lite, and Power Pong rule modules. Its silent menu includes in-game audio enable, volume, and Classic/Teams/Rainbow color settings. Use the D-pad to move, A to select or restart, B to return to the mode menu, and Center to exit.
- **NullpoMino-derived Tetris** - The standalone Tetris app provides 150-line Marathon, 40-Line Race, and two-minute Ultra with Standard, Standard Fast-B, and Nintendo-R rules. It includes hold/next/ghost behavior where supported, SRS kicks, spins, B2B, combos, persistent per-rule records, and silent menus with persistent in-game SFX volume. During play use A/B to rotate, Select to hold, Up to hard-drop, Start to pause, and Center to exit.
- **Source-derived Flappy Bird** - The standalone Flappy Bird app adapts VadimBoev/FlappyBird `v1.9.2` into a silent native DC32 port with the original visible PNG sprites, full-width landscape playfield, source-matched pipe/bird physics, result panel, medals, and `/SAVE/PORTS/flappy.sav` best-score persistence. Use A, Up, or Start to flap/start/restart, and Center to exit.
...
- Tetris, Flappy Bird, T-Rex Runner, Chip's Challenge, Scorched Earth, Pipe Dream, Cave Story, and Sokoban now ship as source-derived/data-compatible ports. Tetris adapts NullpoMino commit `4de098dd0b48d991247313d8dba30b9721e6f9d9` and persists records/settings in `/SAVE/PORTS/tetris.sav`; Flappy Bird adapts VadimBoev/FlappyBird `v1.9.2` and persists its best score in `/SAVE/PORTS/flappy.sav`; T-Rex uses the pinned Wayou/Chromium LDPI sprite and persists its high score in `/SAVE/PORTS/trex.sav`; Cave Story freeware data and `CHIPS.DAT` remain user-provided and are not redistributed.
