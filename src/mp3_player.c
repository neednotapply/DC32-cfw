...
static enum Mp3PlayerControl pollControl(Mp3PlayerControlF controlF, void *userData, struct Mp3PlayerStatus *status)
{
	return controlF ? controlF(userData, status) : Mp3PlayerControlNone;
}

static enum Mp3PlayerResult waitWhilePaused(Mp3PlayerControlF controlF, void *userData, struct Mp3PlayerStatus *status)
{
	...
	audioPwmStop();
	while (1) {
		ctl = pollControl(controlF, userData, status);
...
enum Mp3PlayerResult mp3PlayerPlayFile(struct FatfsFil *fil, Mp3PlayerControlF controlF, void *userData)
{
	...
	enum Mp3PlayerResult pausedResult;
		...
		if (samples > 0 && info.hz > 0 && info.channels > 0) {
			...
			for (i = 0; i < samples; i++) {
				...
				if (!(i & 0x7f)) {
					ctl = pollControl(controlF, userData, &status);
					if (ctl == Mp3PlayerControlPause) {
						pausedResult = waitWhilePaused(controlF, userData, &status);
						if (pausedResult != Mp3PlayerResultDone) {
							audioPwmStop();
							return pausedResult;
						}
					}
