...
bool instr_is_two_words(avr_instr_t i)
{
    return
        ...
        i.func == INSTR_LDS ||
        i.func == INSTR_STS ||
        i.func == INSTR_MERGED_LDS ||
        i.func == INSTR_MERGED_STS;
}
...
static uint32_t embedded_ld32(std::array<uint8_t, atmega32u4_t::DATA_SIZE_BYTES>& data, uint16_t addr)
{
    if(addr == atmega32u4_t::EMBEDDED_HLE_NONE || addr + 3u >= data.size())
        return 0;
    return uint32_t(data[addr + 0u]) |
        (uint32_t(data[addr + 1u]) << 8) |
        (uint32_t(data[addr + 2u]) << 16) |
        (uint32_t(data[addr + 3u]) << 24);
}

static void embedded_st32(std::array<uint8_t, atmega32u4_t::DATA_SIZE_BYTES>& data, uint16_t addr, uint32_t value)
{
    if(addr == atmega32u4_t::EMBEDDED_HLE_NONE || addr + 3u >= data.size())
        return;
    data[addr + 0u] = uint8_t(value >> 0);
    data[addr + 1u] = uint8_t(value >> 8);
    data[addr + 2u] = uint8_t(value >> 16);
...
bool atmega32u4_t::embedded_advance_timer0_delay(uint64_t cycles)
{
    if(cycles == 0)
        return true;
    if(embedded_timer0_overflow_addr == EMBEDDED_HLE_NONE ||
        embedded_timer0_millis_addr == EMBEDDED_HLE_NONE ||
        embedded_timer0_fract_addr == EMBEDDED_HLE_NONE)
        return false;
    if(timer0.divider == 0 || (data[0x64] & (1 << 5)) || timer0.phase_correct ||
    ...
    timer0.tcnt = (uint32_t)(total_ticks & 0xffu);
    data[0x46] = (uint8_t)timer0.tcnt;

    if(overflows)
    {
        uint32_t overflow_count = embedded_ld32(data, embedded_timer0_overflow_addr);
        uint32_t millis = embedded_ld32(data, embedded_timer0_millis_addr);
        uint32_t fract = data[embedded_timer0_fract_addr];
        uint32_t fract_total = fract + overflows * 3u;

        overflow_count += overflows;
        millis += overflows + fract_total / 125u;
        data[embedded_timer0_fract_addr] = (uint8_t)(fract_total % 125u);
        embedded_st32(data, embedded_timer0_overflow_addr, overflow_count);
        embedded_st32(data, embedded_timer0_millis_addr, millis);
        tifr0() &= (uint8_t)~0x01u;
    }

    timer0.prev_update_cycle = cycle_count;
    uint32_t ticks_to_overflow = 256u - timer0.tcnt;
    uint64_t next_cycles = (uint64_t)ticks_to_overflow * timer0.divider;
    if(next_cycles > timer0.prescaler_cycle)
        next_cycles -= timer0.prescaler_cycle;
    else
        next_cycles = 1;
...
uint32_t instr_rcall(atmega32u4_t& cpu, avr_instr_t i)
{
    ...
    uint16_t target = cpu.pc + (int16_t)i.word + 1;
#ifdef ARDENS_EMBEDDED
    if(uint32_t cycles = cpu.embedded_hle_call(target, ret_addr, 3))
        return cycles;
#endif
    cpu.pc = target;
    if(i.word != 0)
...
#ifdef ARDENS_EMBEDDED
    if(uint32_t cycles = cpu.embedded_hle_call(i.word, ret_addr, 4))
        return cycles;
#endif
    cpu.pc = i.word;
    cpu.push_stack_frame(ret_addr);
...
uint32_t instr_merged_dec_brne(atmega32u4_t& cpu, avr_instr_t i)
{
    ...
    if((int16_t)i.word == -2)
    {
        uint32_t loops_total = dst ? dst : 256u;
        uint64_t next_cycle = cpu.peripheral_queue.next_cycle();
        uint64_t safe_cycles = next_cycle > cpu.cycle_count ?
            next_cycle - cpu.cycle_count : 0;
        uint32_t finish_cycles = loops_total * 3u - 1u;

        if(finish_cycles <= safe_cycles)
        {
            cpu.gpr(i.dst) = 0;
            ...
            cpu.sreg() = sreg;
            cpu.pc += 2;
            return finish_cycles;
        }

        uint32_t taken_loops = (uint32_t)std::min<uint64_t>(
            loops_total - 1u, safe_cycles / 3u);
        if(taken_loops > 1u)
        {
            uint8_t res = (uint8_t)(dst - taken_loops);
            cpu.gpr(i.dst) = res;
            unsigned sreg = cpu.sreg() & ~(SREG_V | SREG_N | SREG_Z | SREG_S);
            if(res == 0x7f) sreg |= SREG_V;
            sreg = flags_nzs(sreg, res);
            cpu.sreg() = sreg;
            return taken_loops * 3u;
        }
    }
...
uint32_t instr_merged_sbiw_brne(atmega32u4_t& cpu, avr_instr_t i)
{
    uint16_t dst = cpu.gpr_word(i.dst);
    if(i.src == 1 && (int16_t)i.word == -2)
    {
        uint32_t loops_total = dst ? dst : 65536u;
        uint64_t next_cycle = cpu.peripheral_queue.next_cycle();
        uint64_t safe_cycles = next_cycle > cpu.cycle_count ?
            next_cycle - cpu.cycle_count : 0;
        uint32_t finish_cycles = loops_total * 4u - 1u;

        if(finish_cycles <= safe_cycles)
        {
            cpu.gpr(i.dst + 0) = 0;
            cpu.gpr(i.dst + 1) = 0;
            unsigned sreg = cpu.sreg() & ~(SREG_Z | SREG_V | SREG_C | SREG_N | SREG_S);
            sreg |= SREG_Z;
            cpu.sreg() = (uint8_t)sreg;
            cpu.pc += 2;
            return finish_cycles;
        ...
        uint32_t taken_loops = (uint32_t)std::min<uint64_t>(
            loops_total - 1u, safe_cycles / 4u);
        if(taken_loops > 1u)
        {
            uint16_t res = (uint16_t)(dst - taken_loops);
            cpu.gpr(i.dst + 0) = uint8_t(res >> 0);
            cpu.gpr(i.dst + 1) = uint8_t(res >> 8);
            unsigned sreg = cpu.sreg() & ~(SREG_Z | SREG_V | SREG_C | SREG_N | SREG_S);
            sreg |= (dst & ~res & 0x8000) >> 12;
            sreg |= (res & ~dst & 0x8000) >> 15;
            sreg |= (res & 0x8000) >> 13;
            sreg = flag_s(sreg);
            cpu.sreg() = (uint8_t)sreg;
            return taken_loops * 4u;
        }
    }

    return instr_sbiw(cpu, i);
}
