...
class Game {
	...
	protected:
...
#ifdef DC32_OPENJAZZ
		char           levelFileStorage[16]; ///< Persistent 8.3 level name
		char           pendingBonusFile[16]; ///< Bonus deferred until normal-level cleanup
#endif
		...
		bool      replaceLevelFile (const char *fileName);
...
#ifdef DC32_OPENJAZZ
		void           deferBonusLevel (const char *fileName);
#endif
