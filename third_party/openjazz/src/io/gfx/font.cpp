...
namespace {
#ifdef DC32_OPENJAZZ
	constexpr unsigned int INVALID_FONT_CHAR = 0xffu;
#else
...
SDL_Rect Font::getAtlasRect(unsigned int index) const {
#ifdef DC32_OPENJAZZ
	return {
		atlasRects[index].x, atlasRects[index].y,
		atlasRects[index].w, atlasRects[index].h
	};
#else
	return atlasRects[index];
#endif
}

void Font::commonSetup() {
	characterAtlas = nullptr;
	isOk = false;
	lineHeight = 0;
	spaceWidth = 0;
...
Font::Font (const char* fileName) {
	commonSetup();
#ifdef DC32_OPENJAZZ
	SDL_Surface* cachedAtlas = dc32OjCacheFindSurface(fileName);
#endif
	...
	for (int i = 0; i < MAX_FONT_CHARS; i++) {
		...
		atlasRects[i] = {
#ifdef DC32_OPENJAZZ
			0xff, 0xff, (unsigned char)w, (unsigned char)h
#else
			-1, -1, w, h
#endif
		};
	...
	int aH = 128;
#ifdef DC32_OPENJAZZ
	characterAtlas = cachedAtlas;
	if (!characterAtlas)
		characterAtlas = dc32OjCacheCreateStagingSurface(aW, aH);
#else
	characterAtlas = video.createSurface(nullptr, aW, aH);
#endif
	if (!characterAtlas) {
#ifdef DC32_OPENJAZZ
		dc32OjCacheBuildFailure(fileName, "font staging unavailable");
#endif
	...
	if(res) {
		for (int i = 0; i < nCharacters; i++) {
			...
			if (rects[i].w > 0 && rects[i].h > 0
				...
				SDL_Rect destination = getAtlasRect(i);
				SDL_BlitSurface(chars[i], nullptr, characterAtlas, &destination);
			}
		}
	} else
...
#ifdef DC32_OPENJAZZ
	if (!cachedAtlas &&
			!dc32OjCachePromoteSurfaceChecked(fileName, &characterAtlas))
		throw E_FILE;
#endif
}
...
Font::Font (unsigned char* pixels, bool big) {
	...
	for (int i = 0; i < nCharacters; i++) {
		...
		atlasRects[i] = {
#ifdef DC32_OPENJAZZ
			0xff, 0xff, (unsigned char)charWidth, lineHeight
#else
			-1, -1, charWidth, lineHeight
#endif
		};
	}
	...
	if (!dc32OjCachePromoteSurfaceChecked(
			big ? "PANELBIG" : "PANELSMALL", &characterAtlas))
		throw E_FILE;
#endif
}
...
Font::Font (bool bonus) {
	commonSetup();
#ifdef DC32_OPENJAZZ
	const char* cacheName = bonus ? "BONUSFONT" : "LEVELFONT";
	SDL_Surface* cachedAtlas = dc32OjCacheFindSurface(cacheName);
#endif
	...
	for (int i = 0; i < nCharacters; i++) {
		...
		atlasRects[i] = {
#ifdef DC32_OPENJAZZ
			0xff, 0xff, (unsigned char)width, (unsigned char)height
#else
			-1, -1, width, height
#endif
		};
	...
	if (!characterAtlas) {
#ifdef DC32_OPENJAZZ
		dc32OjCacheBuildFailure(cacheName, "font staging unavailable");
#endif
		for (int i = 0; i < nCharacters; i++)
			if (chars[i])
				video.destroySurface(chars[i]);
...
#ifdef DC32_OPENJAZZ
	if (!cachedAtlas &&
			!dc32OjCachePromoteSurfaceChecked(cacheName, &characterAtlas))
		throw E_FILE;
#endif
}
...
Point Font::showString (const char* string, int x, int y,
	...
	for (int i = 0; string[i]; i++) {
		if (string[i] == '\n') {
			...
			SDL_Rect source = getAtlasRect(c);

			// Draw the character to the screen
			SDL_BlitSurface(characterAtlas, &source, canvas, &dst);
