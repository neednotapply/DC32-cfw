...
class Font {
	...
	private:
		void           commonSetup();
		void           cleanMapping();
		SDL_Rect       getAtlasRect(unsigned int index) const;
		SDL_Surface   *characterAtlas; ///< Symbol images
	#ifdef DC32_OPENJAZZ
		struct {
			unsigned char x, y, w, h;
		} atlasRects[MAX_FONT_CHARS]; ///< Packed symbol positions
	#else
	...
	#ifdef DC32_OPENJAZZ
		unsigned char  map[MAX_FONT_CHARS]; ///< Compact ASCII-to-symbol map
	#else
