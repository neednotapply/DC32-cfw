...
#include <stdio.h>
static_assert(sizeof(JJ1Level) <= DC32_OJ_LEVEL_OBJECT_LIMIT,
	"DC32 JJ1 level state exceeds the fixed level arena");

void* JJ1Level::operator new (size_t size) {
...
void JJ1Level::operator delete (void* ptr) {
...
JJ1Level::JJ1Level (Game* owner) : Level(owner) {
	...
	musicFile = nullptr;
#ifdef DC32_OPENJAZZ
	memset(eventTimes, 0, sizeof(eventTimes));
	memset(eventTimeOwners, 0, sizeof(eventTimeOwners));
#endif
}
...
JJ1Level::JJ1Level (Game* owner, char* fileName, bool checkpoint, bool multi) :
	...
	if (ret < 0) {
		...
		snprintf(detail, sizeof(detail), "%s: %s",
			fileName ? fileName : "level", dc32OjLoadingContext());
		dc32OjLoadingFailure("Level load failed", detail);
#endif
		throw ret;
	}
...
#ifdef DC32_OPENJAZZ
	return (mask[ge->tile][(y >> 12) & 7] >> ((x >> 12) & 7)) & 1;
#else
...
bool JJ1Level::checkMaskDown (fixed x, fixed y) {
	...
	// Check the mask in the tile in question
#ifdef DC32_OPENJAZZ
	return (mask[grid[FTOT(y)][FTOT(x)].tile][(y >> 12) & 7] >>
		((x >> 12) & 7)) & 1;
#else
...
#ifdef DC32_OPENJAZZ
	unsigned char index = grid[gridY][gridX].timeIndex;

	return index ? eventTimes[index] : 0;
#else
...
#ifdef DC32_OPENJAZZ
	GridElement *ge = grid[gridY] + gridX;
	unsigned short owner = (unsigned short)((gridY * LW) + gridX + 1u);
	unsigned char index = ge->timeIndex;

	if (!time) {
		if (index)
			eventTimeOwners[index] = 0;
		ge->timeIndex = 0;
		return;
	}
	if (!index) {
		for (unsigned int candidate = 1; candidate < 256; candidate++)
			if (!eventTimeOwners[candidate]) {
				index = (unsigned char)candidate;
				eventTimeOwners[candidate] = owner;
				ge->timeIndex = index;
				break;
			}
	}
...
int JJ1Level::playBonus () {
	...
	snprintf(bonusFile, sizeof(bonusFile), "BONUSMAP.%03d",
		worldToEpisode(worldNum));
	game->deferBonusLevel(bonusFile);
	return E_NONE;
#else
	char *bonusFile;
	int ret;
