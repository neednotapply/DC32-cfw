...
#ifdef DC32_OPENJAZZ
	unsigned char event : 7; ///< Indexes the event set
	unsigned char bg : 1; ///< 0 = Effect background, 1 = Black background
	unsigned char hits; ///< Number of times the event has been shot
	unsigned char timeIndex; ///< Index into the compact DC32 event-time table
#else
...
#ifdef DC32_OPENJAZZ
static_assert(sizeof(GridElement) == 4, "DC32 compact grid element");
#endif
...
class JJ1Level : public Level {
...
#ifdef DC32_OPENJAZZ
		Sprite        spriteSetStorage[232]; ///< Fixed shareware sprite descriptor pool
#endif
		Anim          animSet[ANIMS]; ///< Animations
#ifdef DC32_OPENJAZZ
		Sprite*       animFrameSprites[531]; ///< Exact bundled-shareware frame pool
		signed char   animFrameX[531];
		signed char   animFrameY[531];
#endif
	...
	#ifdef DC32_OPENJAZZ
		unsigned char mask[240][8]; ///< Packed 8x8 collision masks
	#else
...
#ifdef DC32_OPENJAZZ
		unsigned int  eventTimes[256]; ///< Sparse per-grid event timers
		unsigned short eventTimeOwners[256]; ///< Grid owner for each timer slot
		short int     pathXPool[294]; ///< Bounded bundled-shareware path storage
		short int     pathYPool[294];
#endif
