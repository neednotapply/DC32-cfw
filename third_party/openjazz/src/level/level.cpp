...
#ifdef DC32_OPENJAZZ
	/*
	 * JJ1 cutscenes require at least one writable 320x200 animation surface
	 * plus decoded frames. During a level transition that exceeds the badge's
	 * complete transient heap. Gameplay progression and bonus awards do not
	 * depend on the cinematic, so DC32 follows its startup/planet policy and
	 * advances directly after releasing active palette effects.
	 */
	(void)file;
	return E_NONE;
#else
...
void Level::drawOverlay (unsigned char bg, bool menu, int option,
	...
	if (menu) {
		...
		for (count = 0; count < nMenuOptions; count++) {
...
#ifdef DC32_OPENJAZZ
			int optionY = (canvasH >> 1) + (count << 4) - menuHeight / 2;
			if (count == option) {
				video.drawRect((canvasW >> 1) - 68, optionY - 2, 132, 15,
					selectedTextPalIndex);
				fontmn2->showString(">", (canvasW >> 1) - 66, optionY);
			}
			fontmn2->showString(menuOptions[count], (canvasW >> 1) - 54, optionY);
#else
			...
			fontmn2->showString(menuOptions[count], (canvasW >> 1) - 64,
				(canvasH >> 1) + (count << 4) - menuHeight / 2);
#endif
	...
	switch (option) {
		...
		case 1: // Save
			...
			if (!multiplayer) {
				...
				if (ret >= 0) {
					bool saved = handleSave(ret);
#ifdef DC32_OPENJAZZ
					if (fileMenu.showMessage(saved ? "GAME SAVED" : "SAVE FAILED") == E_QUIT)
						return E_QUIT;
#else
					if (saved)
						playConfirmSound();
...
int Level::loop (bool& menu, int& option, bool& message) {
	...
	if ((menu && menuBackReleased()) ||
			(!menu && controls.release(C_ESCAPE))) {
	...
	if (!menu && controls.release(C_PAUSE)) message = !message;
