...
void startUp (const char *argv0, int pathCount, char *paths[]) {
	...
	SetupOptions config;
#ifdef DC32_OPENJAZZ
	SDL_Surface* rawPanel = nullptr;
#endif

#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Finding game data");
#endif
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Loading configuration");
#endif
	config = setup.load();
	if (!config.valid) {
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Creating display");
#endif
	canvas = NULL;
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Loading fonts");
#endif
	...
	pixels = static_cast<unsigned char*>(rawPanel->pixels);
#else
	...
	video.destroySurface(rawPanel);
	/*
	 * Build the per-level font atlases while SRAM is still unfragmented.
	 * Gameplay constructors later recover only their small layout metadata
	 * and attach the immutable atlas pixels from the QSPI staging cache.
	 */
	{
		Font* cachedLevelFont = new Font(false);
		delete cachedLevelFont;
		Font* cachedBonusFont = new Font(true);
		delete cachedBonusFont;
	}
	if (!dc32OjCacheIsSealed() &&
			!dc32OjCacheCheckpoint("Checking startup cache", 10u,
				"BONUSFONT"))
		throw E_FILE;
#else
	delete[] pixels;
...
#ifdef DC32_OPENJAZZ
	if (!dc32OjCacheIsSealed()) {
		if (!dc32OjPrebuildNormalAssets() || !dc32OjPrebuildBonusAssets())
			throw E_FILE;
		dc32OjLoadingStage("Validating complete asset cache");
		if (!dc32OjCacheSeal())
			throw E_FILE;
	}
#endif
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Preparing game state");
#endif
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Loading startup scene");
#endif
...
#ifdef DC32_OPENJAZZ
	dc32OjLoadingStage("Opening main menu");
#else
	try {
