...
MainMenu::MainMenu () {
	...
	for (int i = 0; i < MAX_PALETTE_COLORS; i++) {
		...
		palette[i].a = 0xff;
		menuPalette[i] = palette[i];
	}
	gameMenu = new GameMenu(nullptr);
	return;
#endif
...
#ifdef DC32_OPENJAZZ
	const char *options[4] = {"new game", "load game", "setup options", "exit"};
	int option = 0;

	video.setPalette(menuPalette);
	while (true) {
		int ret = generic("JAZZ JACKRABBIT", options, 4, option);
