...
int SetupMenu::setupAudio () {
	...
	while (true) {
		...
		if (menuBackReleased()) return E_NONE;

		if (menuConfirmReleased()) return E_NONE;
...
#ifdef DC32_OPENJAZZ
	const char* setupOptions[3] = {"character colors", "audio", "gameplay"};
	const char* colorOptions[4] = {"fur", "bandana", "gun", "wristband"};
	const char* colorNames[8] = {"white", "red", "orange", "yellow",
		"green", "blue", "animation 1", "animation 2"};
	const unsigned char colorValues[8] = {
		PC_GREY, PC_RED, PC_ORANGE, PC_YELLOW,
		PC_LGREEN, PC_BLUE, PC_SANIM, PC_LANIM
	};
	const char* setupModsOff[4] = {"slow motion: off", "extra items: take",
		"bird limit: one", "hud style: classic"};
	const char* setupModsOn[4] = {"slow motion: on", "extra items: leave",
		"bird limit: no", "hud style: old fps"};
	const char* setupMods[4] = {
		setup.slowMotion ? setupModsOn[0] : setupModsOff[0],
		setup.leaveUnneeded ? setupModsOn[1] : setupModsOff[1],
		setup.manyBirds ? setupModsOn[2] : setupModsOff[2],
		setup.hudStyle == hudType::FPS ? setupModsOn[3] : setupModsOff[3]
	};
	int option = 0;

	while (true) {
		int ret = generic("SETUP OPTIONS", setupOptions, 3, option);
		...
		if (option == 0) {
			...
			while (true) {
				ret = generic("CHARACTER COLORS", colorOptions, 4, colorPart);
				if (ret == E_QUIT)
					return E_QUIT;
				if (ret < 0)
					break;
				...
				int color = 0;
				for (int i = 0; i < 8; i++)
					if (setup.characterCols[colorPart] == colorValues[i])
						color = i;
				ret = generic(colorOptions[colorPart], colorNames, 8, color);
				if (ret == E_QUIT)
					return E_QUIT;
				if (ret == E_NONE)
					setup.characterCols[colorPart] = colorValues[color];
			}
		} else if (option == 1) {
			if (setupAudio() == E_QUIT)
				return E_QUIT;
		} else {
			int gameOption = 0;

			while (true) {
				ret = generic("GAME OPTIONS", setupMods, 4, gameOption);
				if (ret == E_QUIT)
					return E_QUIT;
				if (ret < 0)
					break;

				setupMods[gameOption] = setupMods[gameOption] == setupModsOff[gameOption] ?
					setupModsOn[gameOption] : setupModsOff[gameOption];
				...
				setup.hudStyle = setupMods[3] == setupModsOn[3] ?
					hudType::FPS : hudType::Classic;
			}
		}
	}
#else
