...
ssd1306_store_vram_byte(ssd1306_t *part, uint8_t page, uint8_t column, uint8_t value)
{
	uint8_t old = part->vram[page][column];

	if (!old && value)
		part->nonzero_bytes++;
	else if (old && !value)
		part->nonzero_bytes--;
	part->vram[page][column] = value;
	part->flags |= (1 << SSD1306_FLAG_DIRTY);
}

static void SIMAVR_FASTCODE
ssd1306_write_data (ssd1306_t *part)
{
	if (part->addr_mode == SSD1306_ADDR_MODE_PAGE)
	{
		uint8_t column = part->cursor.column;
		ssd1306_store_vram_byte(part, part->cursor.page, column, part->spi_data);
		part->cursor.column = (column + 1u) & (SSD1306_VIRT_COLUMNS - 1u);
		return;
	}

	ssd1306_store_vram_byte(part, part->cursor.page, part->cursor.column, part->spi_data);
...
ssd1306_spi_in_hook (struct avr_irq_t * irq, uint32_t value, void * param)
{
	...
	ssd1306_spi_direct_byte(part, value);
}

void SIMAVR_FASTCODE
ssd1306_spi_direct_byte(void *param, uint8_t value)
{
	ssd1306_t * part = (ssd1306_t*) param;

	part->spi_writes++;
	part->last_spi_byte = value;
...
{
	if (!part || !wiring || !part->avr || !part->irq)
		return;

#if ARDUBOY_SIMAVR_DIRECT_SPI_SSD1306
	avr_spi_fast_output_t fast_spi = {
		.callback = ssd1306_spi_direct_byte,
		.param = part,
	};
	if (avr_ioctl(part->avr, AVR_IOCTL_SPI_SET_FAST_OUTPUT(0), &fast_spi) < 0)
#endif
	avr_connect_irq (
