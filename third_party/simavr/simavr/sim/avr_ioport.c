...
avr_ioport_raise_if_changed(avr_irq_t * irq, uint32_t value)
{
	if (!irq)
		return;
	uint32_t output = (irq->flags & IRQ_FLAG_NOT) ? !value : value;
	if (!(irq->flags & IRQ_FLAG_INIT) && irq->value == output)
		return;
...
static uint8_t SIMAVR_FASTCODE
avr_ioport_read(
		struct avr_t * avr,
		avr_io_addr_t addr,
		void * param)
		...
		avr_ioport_t * p)
{
	...
	if (avr->io[port_io].irq && !p->irqing) {
		avr_ioport_raise_if_changed(avr->io[port_io].irq + AVR_IOMEM_IRQ_ALL, avr->data[p->r_port]);
		for (int i = 0; i < 8; i++)
			avr_ioport_raise_if_changed(avr->io[port_io].irq + i, (avr->data[p->r_port] >> i) & 1);
 	}
}
