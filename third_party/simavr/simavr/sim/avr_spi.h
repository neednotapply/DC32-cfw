...
typedef void (*avr_spi_fast_output_f)(void *param, uint8_t value);

typedef struct avr_spi_fast_output_t {
	avr_spi_fast_output_f callback;
	void *param;
} avr_spi_fast_output_t;

// add port number to get the real IRQ
#define AVR_IOCTL_SPI_GETIRQ(_name) AVR_IOCTL_DEF('s','p','i',(_name))
#define AVR_IOCTL_SPI_SET_FAST_OUTPUT(_name) AVR_IOCTL_DEF('s','p','f',(_name))
...
typedef struct avr_spi_t {
	...
	avr_spi_fast_output_f fast_output;
	void *fast_output_param;
} avr_spi_t;
