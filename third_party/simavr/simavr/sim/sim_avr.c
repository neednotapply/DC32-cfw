		...
		avr_t * avr)
{
	...
	if (!avr->flash) {
		arduboySimAllocationFailure("avr/flash");
		return -1;
	}
	...
	*((uint16_t*)&avr->flash[avr->flashend + 1]) = AVR_OVERFLOW_OPCODE;
	...
	if (!avr->data) {
		arduboySimAllocationFailure("avr/data");
		return -1;
	}
	...
	if (!avr->data_names) {
		arduboySimAllocationFailure("avr/data_names");
		return -1;
	}
#endif
		...
		uint32_t coreLen)
{
	...
	if (!b) {
		arduboySimAllocationFailure("avr/core");
		return NULL;
	}
