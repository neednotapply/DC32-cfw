...
#ifndef SIMAVR_FASTCODE
#define SIMAVR_FASTCODE __attribute__((section(".fastcode")))
#endif
...
void SIMAVR_FASTCODE avr_core_watch_write(avr_t *avr, uint16_t addr, uint8_t v)
{
	if (addr > avr->ramend) {
	...
	if (avr->sram_tracepoint_count)
		_call_sram_irqs(avr, addr);
}

uint8_t SIMAVR_FASTCODE avr_core_watch_read(avr_t *avr, uint16_t addr)
{
	if (addr > avr->ramend) {
		uint16_t ramstart = avr->ioend + 1;
		uint16_t ramsize = avr->ramend - ramstart + 1;
 ...
 */
static inline void SIMAVR_FASTCODE _avr_set_r(avr_t * avr, uint16_t r, uint8_t v)
{
	REG_TOUCH(avr, r);
...
static inline void SIMAVR_FASTCODE
_avr_set_r16le_hl(
	avr_t * avr,
	uint16_t r,
	uint16_t v)
 ...
 */
static inline void SIMAVR_FASTCODE _avr_set_ram(avr_t * avr, uint16_t addr, uint8_t v)
{
	if (addr <= avr->ioend)
		_avr_set_r(avr, addr, v);
	else if (addr <= avr->ramend && !avr->gdb && !avr->sram_tracepoint_count)
		avr->data[addr] = v;
	else
 ...
 */
static inline uint8_t SIMAVR_FASTCODE _avr_get_ram(avr_t * avr, uint16_t addr)
{
	if (addr == R_SREG) {
		/*
	...
	if (addr > avr->ioend && addr <= avr->ramend && !avr->gdb)
		return avr->data[addr];
...
static inline void _avr_push8(avr_t * avr, uint16_t v)
{
	...
	const uint16_t ramStart = avr->ioend + 1;
	if (!avr->gdb && !avr->sram_tracepoint_count &&
			sp >= ramStart &&
			sp <= avr->ramend) {
		avr->data[sp] = v;
		sp--;
		avr->data[R_SPL] = sp & 0xff;
		avr->data[R_SPH] = sp >> 8;
		return;
	}
...
int SIMAVR_FASTCODE _avr_push_addr(avr_t * avr, avr_flashaddr_t addr)
{
	uint16_t sp = _avr_sp_get(avr);
	const int addressSize = avr->address_size;
	const uint16_t ramStart = avr->ioend + 1;
	if (!avr->gdb && !avr->sram_tracepoint_count &&
			sp <= avr->ramend &&
			sp >= (uint16_t)(ramStart + addressSize - 1)) {
		avr_flashaddr_t wordAddr = addr >> 1;
		uint16_t newSp = sp;
		for (int i = 0; i < addressSize; i++, wordAddr >>= 8, newSp--)
			avr->data[newSp] = wordAddr;
		avr->data[R_SPL] = newSp & 0xff;
		avr->data[R_SPH] = newSp >> 8;
		return addressSize;
	}

	addr >>= 1;
	for (int i = 0; i < addressSize; i++, addr >>= 8, sp--)
		_avr_set_ram(avr, sp, addr);
	_avr_sp_set(avr, sp);
	return addressSize;
}

avr_flashaddr_t SIMAVR_FASTCODE _avr_pop_addr(avr_t * avr)
{
	uint16_t sp = _avr_sp_get(avr) + 1;
	...
	if (!avr->gdb && !avr->sram_tracepoint_count &&
			sp >= ramStart &&
			(uint32_t)sp + (uint32_t)addressSize - 1u <= avr->ramend) {
		for (int i = 0; i < addressSize; i++, sp++)
			res = (res << 8) | avr->data[sp];
		res <<= 1;
		avr->data[R_SPL] = (sp - 1) & 0xff;
		avr->data[R_SPH] = (sp - 1) >> 8;
		return res;
	}
 ...
 * entirely accurate.
 */
avr_flashaddr_t SIMAVR_FASTCODE avr_run_one(avr_t * avr)
{
run_one_again:
#if CONFIG_SIMAVR_TRACE
	/*
...
#if ARDUBOY_SIMAVR_BATCH_INTERRUPTS_OFF
		((avr->interrupt_state == 0) || !avr->sreg[S_I]))
#else
