...
def main():
    ...
    expect("image viewer accepts JPEG", '".jpg"' in image_viewer_src and '".jpeg"' in image_viewer_src)
    expect("image viewer accepts BMP", '".bmp"' in image_viewer_src)
    expect("image viewer rejects GIF", '".gif"' not in image_viewer_src)
    expect("picojpeg bridge is built", "picojpeg.c" in cmake_src and "PICOJPEG_SRC" in cmake_src)
    expect("AnimatedGIF bridge is not built", "AnimatedGIF" not in cmake_src and "imageViewerAnimatedGif" not in cmake_src)
    expect("direct BMP decoder exists", "imagePrvRunBmp" in image_viewer_src and "compression != 0u" in image_viewer_src)
    expect("direct JPEG decoder exists", "imagePrvRunJpeg" in image_viewer_src and "pjpeg_decode_mcu" in image_viewer_src)
    ...
    expect("image picker text names general image support", "No image files found in /IMAGES" in ui_src)
    expect("image unsupported text names direct formats", ".jpg, .jpeg, or uncompressed .bmp" in ui_src)
